...
생존코드 등록 전 상담을 통해 관심사의 분리가 중요하다는 홀맨의 말이 떠오른다. 어떠한 관점에서 이를 강조했을까?
강의에서는 리액트 컴포넌트가 UI를 TS가 비즈니스 로직의 담당하는 관심사의 분리를 통해 각각에 대한 테스트 코드를 유려하게 작성할 수 있다는 장점을 설명했다.
...
Layered Architecture에선 사용자에게 가까운 것과 사용자에게 먼 것으로 구분한다.  
가장 가까운 것(UI를 다루는 화면) -> 중간(Business Logic) -> 가장 먼 것(데이터를 읽거나 업데이트하는 부분)
...
3. Persistence layer: 객체 관계형 매핑(ORM)과 같은 기능 처리를 관심사로 둔다.
...
### MVC 패턴 [관련 링크](https://bsnippet.tistory.com/13)

![MVC 패턴](../images/facebook_MVC.png)
...
![MVC 안티패턴](../images/facebook_MVC_anti.png)
...
> 각 구성요소의 역할은 컨트롤러가 비즈니스 로직을 처리하며 뷰와 모델을 연결해주고 모델은 애플리케이션의 데이터를 읽거나 업데이트, 뷰는 사용자가 볼 수 있는 화면으로 이루어져 있는데, Facebook에서 설명하는 패턴이 일반적으로 설명하는 MVC 패턴의 흐름과는 차이가 좀 있다. 혹자는 Facebook에서 잘못된 MVC 패턴을 사용하고 있었다거나, SPA 구조에 MVC 패턴을 억지로 끼워맞추다보니 이런 패턴이 발생했다는 말도 있다. Facebook에서 각각의 구성요소를 어떠한 관점으로 바라보고 있는지 잘 모르겠지만, 프론트엔드 관점에서 컨트롤러는 이벤트를 처리하는 핸들러, 각각의 모델은 컴포넌트의 상태, 뷰를 브라우저에 렌더된 UI라고 생각하면 데이터의 양방향 바인딩이 가져올 시스템 복잡성 증가는 데이터 흐름을 예측하기 어렵겠다는 생각이 든다.
