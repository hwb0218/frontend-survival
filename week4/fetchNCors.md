...
> 정리하자면 ReadableStream은 웹 플랫폼에서 사용되는 Streams API로 대용량의 데이터를 작은 조각(chunk)로 나누어 전송한다. 그래서 필요한만큼 데이터를 순차적으로 읽을 수 있다. 스트리밍이란 단어를 보니 UDP가 떠오르는데 빠르고 가볍게 데이터를 전송하는 전송 계층 프로토콜이 떠오른다. 3-way handshaking 과정이 생략되어 가볍고 빠르게 전송하는 개념의 유사성이 보이긴 하지만, 두가지의 직접적인 연관성은 딱히 없어보인다.
